...
export function Purchase() {
  ...
  const [entries, setEntries] = React.useState([
    { head: 'PURCHASE', amount: '25,000.00' },
    { head: '', amount: '' }
  ]);
  const [narration, setNarration] = React.useState('Being goods purchased from Supplier A on credit.');
  const [date, setDate] = React.useState('2025-03-31');
  const [voucherNo, setVoucherNo] = React.useState('P/001');
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f3f4f6]">
      <div className="bg-[#FFB93B] border-b border-gray-400 shadow-2xl font-sans text-black flex flex-col h-full w-full">
        
        {/* Top Header Bar */}
        <div className="bg-[#556b2f] text-[#f0f0d8] px-2 py-1 flex justify-between items-center text-[13px] font-semibold border-b border-gray-500">
        ...
        <div className="p-3 border-b border-gray-400">
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-4">
              <div className="bg-[#27ae60] text-white px-8 py-0.5 text-[14px] font-bold shadow-sm uppercase">Purchase</div>
            ...
            <div className="text-right leading-tight font-bold text-[14px] relative group">
              ...
              <div className="bg-white border border-gray-400 shadow-sm rounded-sm px-3 py-1 flex items-center gap-3 group-hover:border-gray-600 transition-colors">
                <div className="text-left">
                  <div className="text-[#556b2f] font-black text-[15px] leading-none">{formatTallyDate(date)}</div>
                  <div className="text-[11px] font-bold text-gray-500 uppercase mt-0.5">{getDayName(date)}</div>
                </div>
                <Icons.Calendar className="w-4 h-4 text-[#556b2f] opacity-70" />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-[12px] text-gray-700 italic">
              ...
              <span>45,000.00 Cr</span>
            </div>
          </div>
        </div>

        {/* Particulars Table Header */}
        <div className="grid grid-cols-12 border-b border-black text-[14px] font-bold bg-[#f0f0d8]">
        ...
        <div className="flex-1 flex flex-col relative overflow-y-auto">
                ...
                <div key={idx} className="flex flex-col border-b border-gray-200/30">
                  ...
                  {entry.head && (
                    <div className="px-12 pb-2 text-[11px] text-gray-500 italic">
                      Cur Bal: {Math.floor(Math.random() * 80000).toLocaleString('en-IN')}.00 Dr
                    </div>
                  )}
                </div>
              ))}
            </div>
            <div className="col-span-3 bg-[#f0f0d8]/50">
              {entries.map((entry, idx) => (
          ...
          <div className="mt-auto p-4 border-t border-gray-400 bg-[#f0f0d8] sticky bottom-0">
      ...
      <div className="max-w-6xl mx-auto w-full mt-4 flex justify-end gap-3 pb-4 px-4">
        ...
        <button className="bg-[#556b2f] text-white px-8 py-2 rounded font-bold text-sm hover:bg-[#3d4d21] transition shadow-md flex items-center gap-2">
          <Icons.Check className="w-4 h-4" /> Save Purchase Voucher
        </button>
      </div>
